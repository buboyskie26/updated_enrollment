...
//         && isset($_POST['program_id']) && isset($_POST['civil_status']) 
//         && isset($_POST['nationality']) && isset($_POST['contact_number']) 
//         && isset($_POST['birthday']) && isset($_POST['age']) 
//         && isset($_POST['guardian_name']) 
//         && isset($_POST['guardian_contact_number']) 
//         && isset($_POST['sex']) && isset($_POST['student_status'])
...
//      ) {
                ...
                if($stmt_insert->execute()){
                    ...
                    if($insert_enrollment->execute()){
                        ...
                        if($insert_enrollment->rowCount() > 0){
                            ...
                            echo "Section $program_section Grade $course_level Transferee Student has been confirmed";
                        }
    ...
    && isset($_POST['student_course_level'])
        ...
        $get = $con->prepare("SELECT course_level FROM course
            WHERE course_id=:course_id");
        $get->bindValue(":course_id", $edit_course_id);

        if($get->execute()){

            $course_level = $get->fetchColumn();

            $update_pending = $con->prepare("UPDATE student
            ...
            $update_pending->bindValue(":course_id", $edit_course_id);   
            $update_pending->bindValue(":student_id", $student_id);   
            $update_pending->bindValue(":course_level", $course_level);

            if($update_pending->execute()){

                // AdminUser::success("Successfully Changed", "");
                echo "success";
            }   
        }
