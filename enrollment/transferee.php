...
<?php 
    ...
    if (isset($_POST['transferee_form_submit']) && isset($_POST['IDNO'])) {
		...
		$wasSuccess = $enroll->TransfereeStudentFormSubmit($stud_unique_id, $fname, $lname, $mi, $address,
        ...
        if($wasSuccess){
			...
			if($student_id != 0){
				...
				if($old_enroll->CheckIfStudentEnrolledWithTheSameSchoolYearId($student_id) == false){
					...
					$sql_enrollment_new = $con->prepare("INSERT INTO enrollment 
						(student_id, course_id, school_year_id, enrollment_status, is_new_enrollee, is_transferee)
						VALUES(:student_id, :course_id, :school_year_id, :enrollment_status, :is_new_enrollee, :is_transferee)");
                    ...
                    // Transferee also considers as new_enrolle ? but in student_status = TransfereeP
					$sql_enrollment_new->bindValue(":is_new_enrollee", $is_new_enrollee);
					$sql_enrollment_new->bindValue(":is_transferee", $is_transferee);

					if($sql_enrollment_new->execute()){
						echo "success transferee tentative new enrollee";

						// Redirect to profile page.
						// header("Location: ");
					}else{
						echo "not success of transferee tentative new enrollee";
					}
				}else{
			...
			echo "transferee register failed";
		}
    }
...
<!-- <form action="index.php?q=subject" class="form-horizontal well" method="post" > -->
	...
	<div class="col-md-8"><h2>Transferee</h2></div>
	<div class="col-md-4"><label>Academic Year: <?php echo $currentYear; ?> <span>(<?php echo $current_semester; ?>)</span> </label></div>
		<table class="table">
			...
			<td></td>
				<td colspan="5">	
					<button class="btn btn-success btn-lg" name="transferee_form_submit" type="submit">Submit</button>
				</td>
			</tr>
		</table>
	</div>
